    ...
    <div>
      <div className="grid gap-x-8 gap-y-12 md:grid-cols-12">
        <div className="md:col-span-7">
          <Words
            className="text-[clamp(1.5rem,2.8vw,2.4rem)] font-semibold leading-[1.15] tracking-[-0.025em]"
            segments={[
              {
                t: "Five roles in two years — Tata Technologies, a UK cyber lab, insurance analytics at Go Digit, and now a production SaaS platform. The through-line: I care about",
              },
              { t: "the pixel and the pipeline", hl: true },
              {
                t: "— the interface a person touches, and the data that makes it honest.",
              },
            ]}
          />
          <Reveal delay={200}>
        ...
        <Reveal variant="group" className="md:col-span-4 md:col-start-9">
          <dl className="divide-y divide-rule border-y border-rule">
            {facts.map((fact, i) => (
              <div
                key={fact.key}
                className="sk grid grid-cols-[7rem_1fr] gap-4 py-4"
